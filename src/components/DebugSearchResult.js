...
type DebugSearchResultState = {
  currentTab: string;
};

/**
 * An individual search result for debug view
 */
class DebugSearchResult extends React.Component<DebugSearchResultDefaultProps, DebugSearchResultProps, DebugSearchResultState> {
  static defaultProps = {
    baseUri: '',
    showScores: false,
  ...
  constructor(props: DebugSearchResultProps) {
    super(props);
    this.state = {
      currentTab: DebugSearchResult.getFirstDocumentType(props.document.children),
    };
    (this: any).tabChanged = this.tabChanged.bind(this);
    (this: any).rateDocument = this.rateDocument.bind(this);
  }

  state: DebugSearchResultState;
...
export default Configurable(DebugSearchResult);
